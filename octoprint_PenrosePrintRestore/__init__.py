...
class PenrosePrintRestore(octoprint.plugin.StartupPlugin,
							octoprint.plugin.EventHandlerPlugin,
							octoprint.plugin.SettingsPlugin,
							octoprint.plugin.AssetPlugin,
							octoprint.plugin.TemplatePlugin,
							octoprint.plugin.BlueprintPlugin):
	"""OctoPrint print restore plugin for Penrose."""
	...
	def write_restore_file(self):
		...
		try:
			...
			if "H0" in temps.keys():
				if temps["H0"]["target"] is not None:
					data["barrel0Target"] = temps["H0"]["target"]
			if "H1" in temps.keys():
				if temps["H1"]["target"] is not None:
					data["barrel1Target"] = temps["H1"]["target"]
	...
	def start_restore(self):
		...
		try:
			...
			if data["fileName"] != "None":  # file name is not none
				...
				if "barrel0Target" in data.keys():
					if float(data["barrel0Target"]) > 0:
						self._printer.commands("M104 H0 S100")
				if "barrel1Target" in data.keys():
					if float(data["barrel1Target"]) > 0:
						self._printer.commands("M104 H1 S100")
				if "tool0Target" in data.keys():
					if float(data["tool0Target"]) > 0:
						self._printer.commands(
							"M109 T0 S150")  # just enough heat to remove nozzle without dislodging print
				if "tool1Target" in data.keys():
					if float(data["tool1Target"]) > 0:
						self._printer.commands(
							"M109 T1 S150")  # just enough heat to remove nozzle without dislodging print
				if "barrel0Target" in data.keys():
					if float(data["barrel0Target"]) > 0:
						self._printer.commands("M109 H0 S100")
				if "barrel1Target" in data.keys():
					if float(data["barrel1Target"]) > 0:
						self._printer.commands("M109 H1 S100")
				...
				if "barrel0Target" in data.keys():
					if float(data["barrel0Target"]) > 0:
						self._printer.commands("M104 H0 S{}".format(float(data["barrel0Target"])))
				if "barrel1Target" in data.keys():
					if float(data["barrel1Target"]) > 0:
						self._printer.commands("M104 H1 S{}".format(float(data["barrel1Target"])))
				if float(data["bedTarget"]) > 0:
				...
				if "barrel0Target" in data.keys():
					if float(data["barrel0Target"]) > 0:
						self._printer.commands("M109 H0 S{}".format(float(data["barrel0Target"])))
				if "barrel1Target" in data.keys():
					if float(data["barrel1Target"]) > 0:
						self._printer.commands("M109 H1 S{}".format(float(data["barrel1Target"])))
	...
	def get_update_information(self):
		...
		return dict(
			PenrosePrintRestore=dict(
				displayName="Penrose Print Restore",
				displayVersion=self._plugin_version,
				# version check: github repository
				type="github_release",
				user="FracktalWorks",
				repo="PenrosePrintRestore",
				current=self._plugin_version,
				# update method: pip
				pip="https://github.com/FracktalWorks/PenrosePrintRestore/archive/{target_version}.zip"
			)
		)
# endregion


__plugin_name__ = "Penrose Print Restore"
...
def __plugin_load__():
	...
	__plugin_implementation__ = PenrosePrintRestore()
